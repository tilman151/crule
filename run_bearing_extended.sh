...
poetry run python train.py \
       ...
       +source.reader.split_run_dist="{dev:[1,2,4,5,6,7],val:[3],test:[]}"
poetry run python train.py \
       --multirun hydra/launcher=ray \
       +hydra.launcher.num_gpus=0.5 \
       +task="$FEMTO_TASKS_3" \
       ...
       +logger.tags="[femto,cnn,transductive,extended]" \
       replications=$REPLICATIONS
       +source.reader.split_run_dist="{dev:[1,3],val:[2],test:[]}"
poetry run python train.py \
       --multirun hydra/launcher=ray \
       +hydra.launcher.num_gpus=0.5 \
       +task="$XJTUSY_TASKS" \
       ...
       replications=$REPLICATIONS
       +source.reader.split_run_dist="{dev:[1,2,4,5],val:[3],test:[]}"
done
